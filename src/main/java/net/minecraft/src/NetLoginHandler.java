...
public class NetLoginHandler extends NetHandler {
    ...
    public TcpConnection networkManager;
    ...
    public NetLoginHandler(MinecraftServer minecraftserver, Socket socket, String s) throws java.io.IOException { // CraftBukkit - throws IOException
        ...
        this.networkManager = new TcpConnection(socket, s, this, minecraftserver.E().getPrivate());
    ...
    public void a(Packet2ClientProtocol packet2handshake) {
        // CraftBukkit start
    ...
    public void a(Packet205ClientCommand packet205clientcommand) {
        if (packet205clientcommand.a == 0) {
            if (this.server.getOnlineMode()) {
                (new net.minecraft.src.ThreadLoginVerifier(this, server.server)).start(); // CraftBukkit - add CraftServer
            } else {
                this.i = true;
            }
        }
    ...
    public void d() {
        // CraftBukkit start
        EntityPlayerMP s = this.server.getServerConfigurationManager().attemptLogin(this, this.h, this.hostname);

        if (s == null) {
            return;
            // CraftBukkit end
        } else {
            EntityPlayerMP entityplayer = this.server.getServerConfigurationManager().processLogin(s); // CraftBukkit - this.h -> s
