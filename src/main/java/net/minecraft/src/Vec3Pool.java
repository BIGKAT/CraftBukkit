...
public class Vec3Pool {
    ...
    public final Vec3 create(double d0, double d1, double d2) { // CraftBukkit - add final
        if (this.f == 0) return new Vec3(d0, d1, d2); // CraftBukkit - don't pool objects indefinitely if thread doesn't adhere to contract
        Vec3 vec3d;

        if (this.freelist == null) { // CraftBukkit
            vec3d = new Vec3(d0, d1, d2);
