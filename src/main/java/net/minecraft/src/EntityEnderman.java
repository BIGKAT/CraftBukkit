...
public class EntityEnderman extends EntityMob {
    ...
    protected Entity findTarget() {
        net.minecraft.src.EntityPlayer entityhuman = this.worldObj.findNearbyVulnerablePlayer(this, 64.0D);
    ...
    private boolean d(net.minecraft.src.EntityPlayer entityhuman) {
        ItemStack itemstack = entityhuman.inventory.armor[3];

        if (itemstack != null && itemstack.id == Block.PUMPKIN.blockID) {
            return false;
        } else {
            Vec3 vec3d = entityhuman.i(1.0F).b();
            Vec3 vec3d1 = Vec3.a().create(this.posX - entityhuman.posX, this.boundingBox.b + (double) (this.length / 2.0F) - (entityhuman.posY + (double) entityhuman.getHeadHeight()), this.posZ - entityhuman.posZ);
            double d0 = vec3d1.c();
    ...
    protected boolean c(Entity entity) {
        Vec3 vec3d = Vec3.a().create(this.posX - entity.posX, this.boundingBox.b + (double) (this.length / 2.0F) - entity.posY + (double) entity.getHeadHeight(), this.posZ - entity.posZ);
    ...
    public boolean damageEntity(DamageSource damagesource, int i) {
        if (damagesource instanceof net.minecraft.src.EntityDamageSourceIndirect) {
            for (int j = 0; j < 64; ++j) {
                if (this.n()) {
                    return true;
                }
