...
public class BlockTripwireHook extends Block {
    ...
    public boolean canPlace(World world, int i, int j, int k) {
        return world.isBlockSolidOnSide(i - 1, j, k, SOUTH) ? true : (world.isBlockSolidOnSide(i + 1, j, k, NORTH) ? true : (world.isBlockSolidOnSide(i, j, k - 1, EAST) ? true : world.isBlockSolidOnSide(i, j, k + 1, WEST))); // Forge
    }
    ...
    public int getPlacedData(World world, int i, int j, int k, int l, float f, float f1, float f2, int i1) {
        ...
        // Forge start
        if (l == 2 && world.isBlockSolidOnSide(i, j, k + 1, WEST, true)) {
            b0 = 2;
        }

        if (l == 3 && world.isBlockSolidOnSide(i, j, k - 1, EAST, true)) {
            b0 = 0;
        }

        if (l == 4 && world.isBlockSolidOnSide(i + 1, j, k, NORTH, true)) {
            b0 = 1;
        }

        if (l == 5 && world.isBlockSolidOnSide(i - 1, j, k, SOUTH, true)) {
            b0 = 3;
        }
        // Forge end
    ...
    public void doPhysics(World world, int i, int j, int k, int l) {
        if (l != this.id) {
            if (this.l(world, i, j, k)) {
                ...
                // Forge start
                if (!world.isBlockSolidOnSide(i - 1, j, k, SOUTH) && j1 == 3) {
                    flag = true;
                }

                if (!world.isBlockSolidOnSide(i + 1, j, k, NORTH) && j1 == 1) {
                    flag = true;
                }

                if (!world.isBlockSolidOnSide(i, j, k - 1, EAST) && j1 == 0) {
                    flag = true;
                }

                if (!world.isBlockSolidOnSide(i, j, k + 1, WEST) && j1 == 2) {
                    flag = true;
                }
                // Forge end
