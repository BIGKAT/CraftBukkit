                ...
                for (j1 = i - b0; j1 <= i + b0 && flag; ++j1) {
                    for (k1 = k - b0; k1 <= k + b0 && flag; ++k1) {
                        if (i1 >= 0 && i1 < 256) {
                            ...
                            if (l1 != 0 &&
                            		(Block.byId[l1]!=null && !Block.byId[l1].isLeaves(world, j1, i1, k1)) && 
                            		l1 != Block.GRASS.id && 
                            		l1 != Block.DIRT.id && 
                            		(Block.byId[l1]!=null && !Block.byId[l1].isWood(world, j1, i1, k1)) && 
                            		l1 != Block.SAPLING.id) {
                                flag = false;
                            }
                        } else {
            ...
            for (int l1 = i - k1; l1 <= i + k1 + 1; ++l1) {
                ...
                for (int j2 = j - k1; j2 <= j + k1 + 1; ++j2) {
                    ...
                    if ((i2 >= 0 || k2 >= 0 || i2 * i2 + k2 * k2 <= k1 * k1) &&
                    		...
                    		(random.nextInt(4) != 0 || i2 * i2 + k2 * k2 <= (k1 - 1) * (k1 - 1)) && 
                    		(bl == null || bl.canBeReplacedByLeaves(world, l1, i1, j2))) {
