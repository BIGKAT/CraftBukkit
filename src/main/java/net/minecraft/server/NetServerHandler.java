...
public class NetServerHandler extends NetHandler implements ICommandListener {
	...
	private final static HashSet<Integer> invalidItems = new HashSet<Integer>(Arrays.asList(8, 9, 10, 11, 26, 34, 36,
			51, 52, 55, 59, 60, 63, 64, 68, 71, 75, 78, 83, 90, 92, 93, 94, 95));
	...
	public void a(Packet10Flying packet10flying) {
		...
		if (!this.player.viewingCredits) {
			...
			Location from = new Location(player.getWorld(), lastPosX, lastPosY, lastPosZ, lastYaw, lastPitch); // Get
																												...
																												// Event
																												// location.
			Location to = player.getLocation().clone(); // Start off the To
														// location as the
														// Players current
														// location.
			...
			if (packet10flying.hasPos
					&& !(packet10flying.hasPos && packet10flying.y == -999.0D && packet10flying.stance == -999.0D)) {
				to.setX(packet10flying.x);
				to.setY(packet10flying.y);
				to.setZ(packet10flying.z);
			}

			// If the packet contains look information then we update the To
			// location with the correct Yaw & Pitch.
			...
			if ((delta > 1f / 256 || deltaAngle > 10f) && (this.checkMovement && !this.player.dead)) {
				...
				if (from.getX() != Double.MAX_VALUE) {
					...
					// If the event is cancelled we move the player back to
					// their old location.
					if (event.isCancelled()) {
						this.player.netServerHandler.sendPacket(new Packet13PlayerLookMove(from.getX(),
								from.getY() + 1.6200000047683716D, from.getY(), from.getZ(), from.getYaw(), from
										.getPitch(), false));
						return;
					}

					/*
					 * If a Plugin has changed the To destination then we
					 * teleport the Player there to avoid any 'Moved wrongly' or
					 * 'Moved too quickly' errors. We only do this if the Event
					 * was not cancelled.
					 */
					 ...
					 * during the call of the event. This can happen due to a
					 * plugin teleporting the player instead of using .setTo()
					 */
			...
			if (this.checkMovement && !this.player.dead) {
				...
				float f4 = 0.0625F;
				boolean flag = worldserver.getCubes(this.player,
						this.player.boundingBox.clone().shrink((double) f4, (double) f4, (double) f4)).size() == 0;
				...
				if (d8 > 0.0625D && !this.player.isSleeping() && !this.player.itemInWorldManager.isCreative()) {
					...
					System.out.println("Expected " + this.player.locX + ", " + this.player.locY + ", "
							+ this.player.locZ);
				}

				this.player.setLocation(d1, d2, d3, f2, f3);
				boolean flag2 = worldserver.getCubes(this.player,
						this.player.boundingBox.clone().shrink((double) f4, (double) f4, (double) f4)).size() == 0;
				...
				AxisAlignedBB axisalignedbb = this.player.boundingBox.clone()
						.grow((double) f4, (double) f4, (double) f4).a(0.0D, -0.55D, 0.0D);

				if (!this.minecraftServer.allowFlight && !this.player.abilities.canFly && !worldserver.b(axisalignedbb)) { // CraftBukkit
																															// -
																															// check
																															// abilities
																															// instead
				...
				if (this.player.itemInWorldManager.isCreative())
					return; // CraftBukkit - fixed fall distance accumulating
	...
	public void teleport(Location dest) {
		...
		this.player.netServerHandler.sendPacket(new Packet13PlayerLookMove(d0, d1 + 1.6200000047683716D, d1, d2, f, f1,
				false));
	}
	...
	public void a(Packet14BlockDig packet14blockdig) {
		...
		if (packet14blockdig.e == 4) {
			// CraftBukkit start
			// If the ticks aren't the same then the count starts from 0 and we
			// update the lastDropTick.
			...
			boolean flag = worldserver.weirdIsOpCache = worldserver.dimension != 0
					|| this.minecraftServer.serverConfigurationManager.isOp(this.player.name); // CraftBukkit
			boolean flag1 = false;
			...
			if (packet14blockdig.e == 0) {
				...
				if (i1 < this.server.getSpawnRadius() && !flag) {
					CraftEventFactory.callPlayerInteractEvent(this.player, Action.LEFT_CLICK_BLOCK, i, j, k, l,
	...
	public void a(Packet15Place packet15place) {
		...
		if (packet15place.face == 255) {
			if (packet15place.itemstack != null && packet15place.itemstack.id == this.lastMaterial
					&& this.lastPacket != null && packet15place.timestamp - this.lastPacket < 100) {
		...
		boolean flag1 = worldserver.weirdIsOpCache = worldserver.dimension != 0
				|| this.minecraftServer.serverConfigurationManager.isOp(this.player.name); // CraftBukkit
		...
		if (packet15place.face == 255) {
			...
			PlayerInteractEvent event = CraftEventFactory.callPlayerInteractEvent(this.player, Action.RIGHT_CLICK_AIR,
					itemstack);
			...
			// CraftBukkit end
		} else if (packet15place.b >= this.minecraftServer.t - 1
				&& (packet15place.face == 1 || packet15place.b >= this.minecraftServer.t)) {
			this.player.netServerHandler.sendPacket(new Packet3Chat("\u00A77Height limit for building is "
			...
			if (Math.pow(eyeLoc.getX() - i, 2) + Math.pow(eyeLoc.getY() - j, 2) + Math.pow(eyeLoc.getZ() - k, 2) > Math
					.max(dist, PLACE_DISTANCE_SQUARED)) {
				return;
			}
		...
		if (itemstack == null || itemstack.l() == 0) {
			...
			Slot slot = this.player.activeContainer.a((IInventory) this.player.inventory,
			...
			if (!ItemStack.matches(this.player.inventory.getItemInHand(), packet15place.itemstack) || always) {
				this.sendPacket(new Packet103SetSlot(this.player.activeContainer.windowId, slot.c,
						this.player.inventory.getItemInHand()));
			}
		}
	...
	public void a(String s, Object[] aobject) {
		if (this.disconnected)
			return; // CraftBukkit - rarely it would send a disconnect line
					// twice
	...
	public void a(Packet16BlockItemSwitch packet16blockitemswitch) {
		...
		if (packet16blockitemswitch.itemInHandIndex >= 0
				&& packet16blockitemswitch.itemInHandIndex < PlayerInventory.getHotbarSize()) {
	...
	private void handleCommand(String s) {
		...
		/*
		 * // CraftBukkit start - No longer needed as we have already handled it
		 * in server.dispatchServerCommand above. if
		 * (s.toLowerCase().startsWith("/me ")) { s = "* " + this.player.name +
		 * " " + s.substring(s.indexOf(" ")).trim(); logger.info(s);
		 * this.minecraftServer.serverConfigurationManager.sendAll(new
		 * Packet3Chat(s)); } else if (s.toLowerCase().startsWith("/kill")) {
		 * this.player.damageEntity(DamageSource.GENERIC, 1000); } else if
		 * (s.toLowerCase().startsWith("/tell ")) { String[] astring =
		 * s.split(" ");
		 * 
		 * if (astring.length >= 3) { s = s.substring(s.indexOf(" ")).trim(); s
		 * = s.substring(s.indexOf(" ")).trim(); s = "\u00A77" +
		 * this.player.name + " whispers " + s; logger.info(s + " to " +
		 * astring[1]); if
		 * (!this.minecraftServer.serverConfigurationManager.a(astring[1],
		 * (Packet) (new Packet3Chat(s)))) { this.sendPacket(new
		 * Packet3Chat("\u00A7cThere\'s no player by that name online.")); } } }
		 * else { String s1;
		 * 
		 * if
		 * (this.minecraftServer.serverConfigurationManager.isOp(this.player.name
		 * )) { s1 = s.substring(1); logger.info(this.player.name +
		 * " issued server command: " + s1);
		 * this.minecraftServer.issueCommand(s1, this); } else { s1 =
		 * s.substring(1); logger.info(this.player.name + " tried command: " +
		 * s1); } } // CraftBukkit end
		 */
	}
	...
	public void a(Packet18ArmAnimation packet18armanimation) {
		...
		if (packet18armanimation.b == 1) {
			...
			double d1 = this.player.lastY + (this.player.locY - this.player.lastY) * (double) f + 1.62D
	...
	public void a(Packet19EntityAction packet19entityaction) {
		...
		if (packet19entityaction.animation == 1 || packet19entityaction.animation == 2) {
			PlayerToggleSneakEvent event = new PlayerToggleSneakEvent(this.getPlayer(),
					packet19entityaction.animation == 1);
			this.server.getPluginManager().callEvent(event);
		...
		if (packet19entityaction.animation == 4 || packet19entityaction.animation == 5) {
			PlayerToggleSprintEvent event = new PlayerToggleSprintEvent(this.getPlayer(),
					packet19entityaction.animation == 4);
			this.server.getPluginManager().callEvent(event);
		...
		if (entity != null) {
			...
			if (this.player.j(entity) < d0) {
				...
				if (packet7useentity.action == 0) {
					// CraftBukkit start
					PlayerInteractEntityEvent event = new PlayerInteractEntityEvent((Player) this.getPlayer(),
					...
					// CraftBukkit start - update the client if the item is an
					// infinite one
					...
					if ((entity instanceof EntityItem) || (entity instanceof EntityExperienceOrb)
						...
						System.out.println("Player " + player.name + " tried to attack an " + type
								+ ", so I have disconnected them for exploiting.");
						return;
					}
	...
	public void a(Packet9Respawn packet9respawn) {
		if (this.player.viewingCredits) {
			...
			if (this.player.getBukkitEntity().getBedSpawnLocation() == null) {
				...
				toLocation = new Location(cworld, chunkcoordinates.x + 0.5, chunkcoordinates.y,
						chunkcoordinates.z + 0.5);
				this.player.netServerHandler.sendPacket(new Packet70Bed(0, 0));
			} else {
				toLocation = this.player.getBukkitEntity().getBedSpawnLocation();
				toLocation = new Location(toLocation.getWorld(), toLocation.getX() + 0.5, toLocation.getY(),
						toLocation.getZ() + 0.5);
			}

			PlayerPortalEvent event = new PlayerPortalEvent(this.player.getBukkitEntity(), this.player
					.getBukkitEntity().getLocation(), toLocation, pta, PlayerPortalEvent.TeleportCause.END_PORTAL);
		...
		this.player.viewingCredits = false; // allow the player to receive
											// movement packets again.
		// CraftBukkit end
	}
	...
	public void a(Packet102WindowClick packet102windowclick) {
		...
		if (this.player.activeContainer.windowId == packet102windowclick.a
			...
			InventoryClickEvent event = new InventoryClickEvent(inventory, type, packet102windowclick.slot,
					packet102windowclick.button != 0, packet102windowclick.shift);
			...
			if (packet102windowclick.slot == 0 && top instanceof CraftingInventory) {
				...
				if (recipe != null) {
					event = new CraftItemEvent(recipe, inventory, type, packet102windowclick.slot,
							packet102windowclick.button != 0, packet102windowclick.shift);
				}
			}
			server.getPluginManager().callEvent(event);
			...
			case DEFAULT:
				itemstack = this.player.activeContainer.clickItem(packet102windowclick.slot,
						packet102windowclick.button, packet102windowclick.shift, this.player);
			...
			if (defaultBehaviour && ItemStack.matches(packet102windowclick.item, itemstack)) { // CraftBukkit
																								// -
																								// additional
																								// condition
																								// added
				this.player.netServerHandler.sendPacket((Packet) (new Packet106Transaction(packet102windowclick.a,
						packet102windowclick.d, true)));
				this.player.h = true;
				this.player.activeContainer.a();
				...
				this.player.netServerHandler.sendPacket(new Packet106Transaction(packet102windowclick.a,
						packet102windowclick.d, false));
				...
				// CraftBukkit start - send a Set Slot to update the crafting
				// result slot
				if (type == SlotType.RESULT && itemstack != null)
					this.player.netServerHandler.sendPacket((Packet) (new Packet103SetSlot(
							this.player.activeContainer.windowId, 0, itemstack)));
				// CraftBukkit end
			}
		}
	}
	...
	public void a(Packet108ButtonClick packet108buttonclick) {
		if (this.player.activeContainer.windowId == packet108buttonclick.a
				&& this.player.activeContainer.c(this.player)) {
	...
	public void a(Packet107SetCreativeSlot packet107setcreativeslot) {
		if (this.player.itemInWorldManager.isCreative()) {
			...
			boolean flag1 = packet107setcreativeslot.slot >= 36
					&& packet107setcreativeslot.slot < 36 + PlayerInventory.getHotbarSize();
			// CraftBukkit
			boolean flag2 = itemstack == null || itemstack.id < Item.byId.length && itemstack.id >= 0
					&& Item.byId[itemstack.id] != null && !invalidItems.contains(itemstack.id);
			boolean flag3 = itemstack == null || itemstack.getData() >= 0 && itemstack.getData() >= 0
					&& itemstack.count <= 64 && itemstack.count > 0;
			...
			InventoryClickEvent event = new InventoryClickEvent(inventory, slot, slot == SlotType.OUTSIDE ? -999
					: packet107setcreativeslot.slot, false, false);
			server.getPluginManager().callEvent(event);
			org.bukkit.inventory.ItemStack item = event.getCurrentItem();
			...
			case ALLOW:
				if (slot == SlotType.QUICKBAR) {
					if (item == null) {
						...
						this.player.defaultContainer.setItem(packet107setcreativeslot.slot,
			...
			case DENY:
				...
				if (packet107setcreativeslot.slot > -1) {
					this.player.netServerHandler.sendPacket(new Packet103SetSlot(this.player.defaultContainer.windowId,
							packet107setcreativeslot.slot, CraftItemStack.createNMSItemStack(item)));
				}
				this.player.netServerHandler.sendPacket(new Packet103SetSlot(this.player.defaultContainer.windowId, -1,
						null));
	...
	public void a(Packet106Transaction packet106transaction) {
		...
		if (oshort != null && packet106transaction.b == oshort.shortValue()
				&& this.player.activeContainer.windowId == packet106transaction.a
				&& !this.player.activeContainer.c(this.player)) {
	...
	public void a(Packet130UpdateSign packet130updatesign) {
		...
		if (worldserver.isLoaded(packet130updatesign.x, packet130updatesign.y, packet130updatesign.z)) {
			TileEntity tileentity = worldserver.getTileEntity(packet130updatesign.x, packet130updatesign.y,
			...
			if (tileentity instanceof TileEntitySign) {
				...
				if (!tileentitysign.c()) {
					this.minecraftServer.warning("Player " + this.player.name
							+ " just tried to change non-editable sign");
					// CraftBukkit
					this.sendPacket(new Packet130UpdateSign(packet130updatesign.x, packet130updatesign.y,
							packet130updatesign.z, tileentitysign.lines));
					return;
				}
			}
			...
			if (tileentity instanceof TileEntitySign) {
				...
				SignChangeEvent event = new SignChangeEvent((CraftBlock) player.getWorld().getBlockAt(j, k, i),
						this.server.getPlayer(this.player), packet130updatesign.lines);
				this.server.getPluginManager().callEvent(event);
	...
	public void a(Packet250CustomPayload packet) {
		...
		if (packet.tag.equals("REGISTER")) {
			try {
				...
				Logger.getLogger(NetServerHandler.class.getName()).log(Level.SEVERE,
						"Could not parse REGISTER payload in plugin message packet", ex);
			}
			...
			try {
				...
				Logger.getLogger(NetServerHandler.class.getName()).log(Level.SEVERE,
						"Could not parse UNREGISTER payload in plugin message packet", ex);
			}
		} else {
