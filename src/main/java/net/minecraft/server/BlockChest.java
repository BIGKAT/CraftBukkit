...
public class BlockChest extends BlockContainer {
    ...
    public boolean interact(World world, int i, int j, int k, EntityHuman entityhuman) {
        ...
        if (object == null) {
            return true;
        // FORGE:
        } else if (world.isBlockSolidOnSide(i, j + 1, k,0)) {
            return true;
        } else if (world.getTypeId(i - 1, j, k) == this.id && (world.isBlockSolidOnSide(i - 1, j + 1, k,0) || h(world, i - 1, j, k))) {
            return true;
        } else if (world.getTypeId(i + 1, j, k) == this.id && (world.isBlockSolidOnSide(i + 1, j + 1, k,0) || h(world, i - 1, j, k))) {
            return true;
        } else if (world.getTypeId(i, j, k - 1) == this.id && (world.isBlockSolidOnSide(i, j + 1, k - 1,0) || h(world, i - 1, j, k))) {
            return true;
        } else if (world.getTypeId(i, j, k + 1) == this.id && (world.isBlockSolidOnSide(i, j + 1, k + 1,0) || h(world, i - 1, j, k))) {
            return true;
        } else {
